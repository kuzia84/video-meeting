...
> **Статус.** Реализованы фундамент и первый вертикальный срез: аутентификация (регистрация/вход по JWT), создание и просмотр встреч и веб-интерфейс (страницы регистрации, входа и защищённой главной со списком встреч). Транскрипция и ИИ-анализ — в планах.
...
- 🖥️ **Веб-интерфейс** — страницы регистрации и входа, защищённая главная с приветствием, счётчиком встреч и тремя последними встречами (серверная пагинация).
...
**Секреты, ключи и учётные данные никогда не коммитятся в репозиторий.**

- Реальные значения храните только в `.env`-файлах — они в `.gitignore` (игнорируются любые `.env`/`.env.*`, а также `*.pem`, `*.key`, `*.p12`, `*.crt`, `service-account*.json` и т.п.). В git попадают **только шаблоны `*.example`** с placeholder-значениями.
- Настройка окружения = скопировать шаблон и подставить свои значения: `cp apps/api/.env.example apps/api/.env`. При добавлении/переименовании переменной обновляйте соответствующий `.example` в том же изменении.
- Префикс `NEXT_PUBLIC_*` делает переменную **публичной** — Next.js вшивает её в клиентский бандл. Секреты под этот префикс класть нельзя; серверные секреты (`JWT_SECRET`, `DATABASE_URL`) живут в `apps/api/.env` и читаются только на сервере.
- В production передавайте секреты через окружение или менеджер секретов (env-конфиг платформы, Docker/Kubernetes secrets, vault), задайте стойкий уникальный `JWT_SECRET`. Если секрет всё же попал в репозиторий — **смените (ротируйте) его** и вычистите из истории: добавить в `.gitignore` постфактум недостаточно, git хранит его в прошлых коммитах.
...
- **`/register`** — регистрация нового пользователя, ссылка на вход.
