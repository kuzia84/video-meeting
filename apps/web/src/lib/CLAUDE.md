...
## Date formatters (`format-date.ts`)

Shared `ru-RU` `Intl.DateTimeFormat` wrappers — `formatMeetingDate` (day/month/time, the home card), `formatMeetingDateTime` (adds year, the meeting header), `formatFileDate` (day/month/year, a file's upload date). One formatter instance per shape, created once at module load; use these rather than re-declaring an `Intl.DateTimeFormat` per screen (the three shapes had drifted when each page kept its own).
