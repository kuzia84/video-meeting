...
- **Pages & auth gating**: `/register` and `/login` are public (server `page.tsx` renders the logo/tagline + a `'use client'` form — mirror one when adding auth screens). `/` is the authenticated home (`page.tsx` → `home-view.tsx`, `'use client'`): greeting (email decoded from the JWT), meeting count, the three most recent meetings or an empty state, a "Создать встречу" CTA, and logout. **Route protection is client-side** — the token lives in `sessionStorage` (invisible to the server, so there's no `middleware.ts`); the home view checks `getAccessToken()` in a mount effect and `router.replace('/login')` when absent (and again on a `401` from the API, after clearing the dead token). New protected pages follow this same in-effect guard.
