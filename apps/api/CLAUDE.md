...
- **Meeting file upload (planned, not implemented)**: storing an uploaded recording against a meeting (`POST /meetings/:id/files`, `GET /meetings/:id/files`, `GET /meetings/:id/files/:fileId`) is specified but not built yet. Read **`docs/meeting-file-upload-research.md`** before writing any of it — it is the technical ground truth for this work and records decisions that are wrong-by-default if you follow the framework docs literally: the 100 MB cap belongs in multer's `limits.fileSize` (a `ParseFilePipe` validator only runs _after_ the whole oversized file is on disk); the meeting-owner check belongs in a guard, since guards run before interceptors and a handler-level check lets a non-owner write 100 MB before their 404; multer writes the file before the handler, so every post-write failure orphans a file unless it is explicitly unlinked; Nest maps `LIMIT_FILE_SIZE` to `PayloadTooLargeException` with the message `File too large`, which does not name the limit the PRD requires; and busboy hands `originalname` over as latin1, so Cyrillic filenames need re-decoding. `@types/multer` is not installed yet. Requirements live in `docs/meeting-file-upload.md`, the phased plan in `docs/meeting-file-upload-plan.md`, and the backlog is milestones 1–5 in GitHub issues.
...
- **Config (`main.ts`, `app.module.ts`)**: `ConfigModule.forRoot({ isGlobal: true })` reads `apps/api/.env` (see `.env.example`: `DATABASE_URL`, `JWT_SECRET`, `JWT_EXPIRES_IN` default `1h`, `CORS_ORIGINS` default `http://localhost:3000`). `main.ts` also installs a global `ValidationPipe({ whitelist: true, transform: true })`.
